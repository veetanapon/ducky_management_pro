...
/* ===== js/modules/system-health-page.js ===== */
(() => {
  const $ = (id) => document.getElementById(id);
  const esc = (value) => String(value ?? '').replace(/[&<>"']/g, (c) => ({ '&': '&amp;', '<': '&lt;', '>': '&gt;', '"': '&quot;', "'": '&#039;' }[c]));

  const getSelectedMode = () => document.querySelector('input[name="dataSourceMode"]:checked')?.value || 'supabase_full';
  const setSelectedMode = (mode) => {
    document.querySelectorAll('input[name="dataSourceMode"]').forEach((input) => { input.checked = input.value === mode; });
  };

  const modeLabel = (mode) => {
    if (mode === 'sheet_only') return 'Sheet fallback';
    if (mode === 'supabase_full') return 'Supabase full';
    return mode || '-';
  };

  const statusTone = (value) => {
    const s = String(value ?? '').toLowerCase();
    if (value === true || ['ok', 'success', 'supabase_full', 'connected'].includes(s)) return 'ok';
    if (value === false || ['error', 'failed', 'missing'].includes(s)) return 'bad';
    if (['sheet_only', 'warn', 'warning', 'fallback', 'partial'].includes(s)) return 'warn';
    return 'neutral';
  };
  ...
  function setBusy(isBusy) {
    document.querySelectorAll('.health-icon-action').forEach((btn) => { btn.disabled = !!isBusy; });
    $('healthStatusBadge')?.classList.toggle('is-loading', !!isBusy);
  }

  function setResult(title, message, tone = 'neutral') {
    const card = $('healthResultCard');
    if (!card) return;
    const iconMap = { ok: '✓', warn: '!', bad: '×', neutral: 'i' };
    card.className = `health-result-card result-${tone}`;
    card.innerHTML = `
      <div class="health-result-icon">${esc(iconMap[tone] || 'i')}</div>
      <div>
        <b>${esc(title || '-')}</b>
  ...
  function renderMode(mode, data = {}) {
    const currentMode = mode || 'supabase_full';
    const tone = statusTone(currentMode);
    const badge = $('modeBadge');
    const hint = $('modeHint');
    const dot = $('modeDot');
    const desc = $('modeDescription');
    const status = $('healthStatusBadge');

    if (badge) {
      badge.textContent = modeLabel(currentMode);
      badge.className = `mode-badge mode-${tone}`;
    }
    if (hint) hint.textContent = currentMode === 'sheet_only' ? 'Google Sheets fallback' : 'Supabase primary';
    if (dot) dot.className = `mode-dot mode-${tone}`;
    if (desc) desc.textContent = currentMode === 'sheet_only'
      ? 'ระบบกำลัง fallback ไปใช้ Google Sheets ชั่วคราว'
      : 'ระบบกำลังใช้ Supabase เป็นฐานข้อมูลหลัก';

    if (status) {
      const statusText = data.status || 'ok';
      status.textContent = statusText;
      status.className = `badge-inline status-${statusTone(statusText || currentMode)}`;
    }
    setSelectedMode(currentMode);
  }

  function renderHealthResult(res = {}) {
    const mode = res.mode || res.data_source_mode || getSelectedMode();
    const source = res.source || 'gas';
    const responseMs = res.response_time_ms || res.supabase_response_time_ms || res.elapsed_ms || '';
    const isOk = res.status === 'ok' || res.connected === true || res.ok === true;
    const tone = isOk ? 'ok' : 'warn';
    const msgParts = [];
    msgParts.push(`Mode: ${modeLabel(mode)}`);
    msgParts.push(`Source: ${source}`);
    if (responseMs) msgParts.push(`Response: ${responseMs} ms`);
    setResult(isOk ? 'ระบบพร้อมใช้งาน' : 'ตรวจพบสถานะที่ควรตรวจสอบ', msgParts.join(' · '), tone);

    const badge = $('lastCheckedBadge');
    if (badge) {
      badge.textContent = `ตรวจล่าสุด ${new Date().toLocaleTimeString('th-TH', { hour: '2-digit', minute: '2-digit' })}`;
      badge.className = `badge-inline ${isOk ? 'status-ok' : 'status-warn'}`;
    }
  }

  async function loadMode() {
    const res = await DuckyAdminService.getMode();
    const mode = res?.mode || res?.data_source_mode || 'supabase_full';
    renderMode(mode, res || {});
    setResult('โหลดโหมดสำเร็จ', `โหมดปัจจุบันคือ ${modeLabel(mode)}`, statusTone(mode));
    return res;
  }
  ...
  async function loadHealth({ silent = false } = {}) {
    try {
      setBusy(true);
      if (!silent) toast('กำลังตรวจสถานะระบบ...', 'info');
      const res = await DuckyAdminService.health({ include_validation: false, lightweight: true });
      renderMode(res?.mode || res?.data_source_mode || getSelectedMode(), res || {});
      renderHealthResult(res || {});
      if (res?.status === 'ok') toast('ตรวจสถานะสำเร็จ', 'success');
      else toast(window.DuckyUI?.formatError?.(res, 'ตรวจสถานะไม่สำเร็จ') || 'ตรวจสถานะไม่สำเร็จ', 'error');
      return res;
    } catch (err) {
      setResult('ตรวจสถานะไม่สำเร็จ', err?.message || String(err), 'bad');
      toast(err?.message || 'ตรวจสถานะไม่สำเร็จ', 'error');
      throw err;
    } finally {
      setBusy(false);
    }
  ...
  async function saveMode(mode) {
    const modeToSave = mode || getSelectedMode();
    if (modeToSave === 'sheet_only' && !confirm('ต้องการ fallback ไป Google Sheets ชั่วคราวใช่ไหม?')) return;
    try {
      setBusy(true);
      const res = await DuckyAdminService.setMode(modeToSave);
      renderMode(res?.mode || modeToSave, res || {});
      setResult('บันทึกโหมดสำเร็จ', `เปลี่ยนเป็น ${modeLabel(res?.mode || modeToSave)} แล้ว`, statusTone(res?.mode || modeToSave));
      toast(`เปลี่ยนโหมดเป็น ${modeLabel(res?.mode || modeToSave)} แล้ว`, res?.status === 'ok' ? 'success' : 'error');
  ...
  document.addEventListener('DOMContentLoaded', async () => {
    if (!(await AppAuth.ensureAuth())) return;
    await loadMode();
    await loadHealth({ silent: true });
    $('healthRefreshIcon')?.addEventListener('click', () => loadHealth());
    $('refreshModeBtn')?.addEventListener('click', async () => { await loadMode(); });
    $('saveModeBtn')?.addEventListener('click', () => saveMode());
    $('healthBtn')?.addEventListener('click', () => loadHealth());
    $('rollbackBtn')?.addEventListener('click', () => saveMode('sheet_only'));
  });
})();
