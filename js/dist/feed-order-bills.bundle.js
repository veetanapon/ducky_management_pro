...
window.FeedOrderService = (() => {
  ...
  function getPageData(options = {}) {
    ...
    return postAction([
      ...
      include_hidden: include_hidden == null ? 0 : include_hidden,
      sort_by: 'purchase_date',
      sort_dir: 'asc'
    }, { timeoutMs: 30000, ...apiOptions });
  }
  ...
  function saveLot(payload) {
    return postAction([
      ...
      'createFeedOrderBill',
      'createFeedOrderLot'
    ], payload, { timeoutMs: 90000, dedupe: false });
  }
  ...
  function allocate(payload) {
    ...
    ], normalizeLotPayload(payload), { timeoutMs: 90000, dedupe: false });
  }
...
window.FeedOrderBillsPage = (() => {
  const CACHE_KEY = 'ducky:feed-order:page-data:v1';
  const CACHE_TTL_MS = 2 * 60 * 1000;
  const CACHE_MAX_STALE_MS = 24 * 60 * 60 * 1000;

  let pageDataUnsubscribe = null;
  ...
  function readLot(raw = {}) {
    ...
    const allocationDestinations = allocations
      .map((a) => {
        ...
        const date = dateOnly(a.allocation_date || a.log_date || '');
    ...
    const paymentRows = payments
      .map((p) => ({
        payment_date: dateOnly(p.payment_date || p.date || ''),
    ...
    const latestPaymentDate = dateOnly(raw.last_payment_date || raw.payment_date_latest || latestPayment?.payment_date || '');
    ...
    return {
      ...
      title,
      supplier,
      purchaseDate: dateOnly(raw.purchase_date || raw.log_date || raw.date || ''),
      purchaseQty,
      returnedQty,
      replacementQty,
      netQty,
  ...
  function normalizePayload(res = {}) {
    const rows = sortLotsOldestFirst((res.lots || res.bills || res.rows || res.feed_order_bills || [])
      .map(readLot)
  ...
  function bindBackgroundUpdates() {
    if (pageDataUnsubscribe || !window.AppSectionLoader?.subscribeMany) return;
    pageDataUnsubscribe = AppSectionLoader.subscribeMany([
      'getFeedOrderBillPageData',
      'getFeedOrderBillsPageData',
      'getFeedOrderPageData'
    ], (response) => {
      if (!response || response.status !== 'ok') return;
      writePageCache(response);
      normalizePayload(response);
      render();
      AppSectionLoader.applyCachedBadge('feedOrderList', response);
      AppSectionLoader.applyCachedBadge('feedOrderSummary', response);
    });
  }
  ...
  async function load({ force = false } = {}) {
    ...
    if (!force && cached.hasValue && cached.ageMs <= CACHE_MAX_STALE_MS) {
      normalizePayload(cached.value);
      state.lastUpdated = cached.value?.generated_at || cached.value?.updated_at || `cache ${Math.max(1, Math.round(cached.ageMs / 1000))} วิที่แล้ว`;
      render();
      AppSectionLoader?.applyCachedBadge?.('feedOrderList', { __from_cache: true, __cache_age_ms: cached.ageMs, __cache_stale: cached.isStale });
      AppSectionLoader?.applyCachedBadge?.('feedOrderSummary', { __from_cache: true, __cache_age_ms: cached.ageMs, __cache_stale: cached.isStale });
      setSubtitle(cached.isStale ? 'แสดงข้อมูลจาก cache และกำลังอัปเดต...' : 'แสดงข้อมูลล่าสุดจาก cache และกำลังตรวจสอบอัปเดต...');
      refreshPageData({ blocking: false });
      return;
    }

    setSubtitle(force ? 'กำลังโหลดใหม่...' : 'กำลังโหลดข้อมูล...');
    await refreshPageData({ blocking: true });
  }

  function readPageCache() {
    if (!window.AppCache?.readEnvelopeDetailed) {
      return { hasValue: false, value: null, isStale: false, ageMs: Infinity };
    }
    return AppCache.readEnvelopeDetailed(CACHE_KEY, CACHE_TTL_MS, null, { allowStale: true, withMeta: true });
  }

  function writePageCache(res) {
    if (res?.status === 'ok' && window.AppCache?.writeEnvelope) AppCache.writeEnvelope(CACHE_KEY, res);
  }

  async function refreshPageData({ blocking = false } = {}) {
    const btn = $('reloadBtn');
    const runner = async () => {
      AppSectionLoader?.mark?.('feedOrderList', blocking ? 'loading' : 'syncing', blocking ? 'กำลังโหลดรายการ...' : 'กำลังอัปเดต...');
      AppSectionLoader?.mark?.('feedOrderSummary', blocking ? 'loading' : 'syncing', blocking ? 'กำลังโหลดสรุป...' : 'กำลังอัปเดต...');
      const res = await FeedOrderService.getPageData({ include_hidden: 0, dedupe: false });
      if (!res || res.status !== 'ok') {
        if (blocking) setSubtitle(res?.message || 'โหลดข้อมูลไม่สำเร็จ');
        else setSubtitle('อัปเดตไม่สำเร็จ กำลังแสดงข้อมูลจาก cache');
        AppSectionLoader?.mark?.('feedOrderList', 'stale', 'อัปเดตไม่สำเร็จ');
        AppSectionLoader?.mark?.('feedOrderSummary', 'stale', 'อัปเดตไม่สำเร็จ');
        return;
      }
      writePageCache(res);
      normalizePayload(res);
      render();
      AppSectionLoader?.applyCachedBadge?.('feedOrderList', res);
      AppSectionLoader?.applyCachedBadge?.('feedOrderSummary', res);
    };

    if (blocking) {
      await withButton(btn, 'กำลังโหลด...', runner);
      return;
    }

    runner().catch((error) => {
      console.warn('Feed order background refresh failed', error);
      setSubtitle('อัปเดตไม่สำเร็จ กำลังแสดงข้อมูลจาก cache');
    });
  }
  ...
  function sortLotsOldestFirst(rows = []) {
    return rows.slice().sort((a, b) => {
      const dateA = String(a.purchaseDate || '9999-12-31');
      const dateB = String(b.purchaseDate || '9999-12-31');
  ...
  function renderLotCard(row) {
    ...
    const expandedContent = `
        <div class="feed-order-expanded-toolbar">
          <button type="button" class="feed-order-show-less" data-feed-action="toggle" data-id="${esc(row.id)}" aria-expanded="true" aria-label="ซ่อนรายละเอียดล็อตอาหาร">
            ลด ▲
          </button>
        </div>
      ...
      <article class="feed-order-card ${isExpanded ? 'is-expanded' : 'is-collapsed'}" data-lot-id="${esc(row.id)}">
        <div class="feed-order-card__head">
          <div class="feed-order-title-wrap">
            <h3>${esc(row.title)}</h3>
            <div class="feed-order-card__meta">${esc([row.purchaseDate, row.supplier].filter(Boolean).join(' • ') || '-')}</div>
          </div>
  ...
  function openAllocateSheet(lot) {
    openSheet(`แบ่งเข้าเล้า • ${lot.title}`, `
      <form id="feedOrderAllocateForm" class="feed-order-form">
        ...
        <div class="feed-order-form-grid">
          <label>จำนวนที่แบ่ง<input name="qty_allocated" type="number" min="0" step="0.01" max="${esc(Math.max(0, n(lot.netQty) - n(lot.allocated)))}" required></label>
        ...
        sheet.querySelector('#feedOrderAllocateForm')?.addEventListener('submit', async (e) => {
          e.preventDefault();
          const qty = Number(e.currentTarget.querySelector('input[name="qty_allocated"]')?.value || 0);
          const remain = Math.max(0, n(lot.netQty) - n(lot.allocated));
          if (!(qty > 0)) return alert('จำนวนที่แบ่งต้องมากกว่า 0');
          if (qty - remain > 0.000001) return alert(`แบ่งเกินจำนวนคงเหลือของ lot นี้ (คงเหลือ ${fmt(remain, 2)} ลูก)`);
  ...
  function renderBulkPaymentPlan(plan = {}) {
    ...
    return `
      <div class="feed-order-bulk-plan-head">
        <div class="feed-order-bulk-plan-group"><span>รหัสกลุ่มจ่าย</span><strong>${esc(plan.payment_group_id || '-')}</strong></div>
        <div class="feed-order-bulk-plan-total"><span>ยอดที่จะหักจริง</span><strong>${money(plan.applied_amount || 0)} บาท</strong></div>
      </div>
    ...
    openSheet('จ่ายยอดรวม / Preview ก่อนบันทึก', `
      <form id="feedOrderBulkPaymentForm" class="feed-order-form">
        ...
        <div class="sheet-footer-row feed-order-bulk-footer">
          <button type="button" class="btn secondary feed-order-bulk-preview-btn" data-preview-bulk-payment>ดูตัวอย่าง</button>
          <button type="button" class="btn secondary feed-order-bulk-cancel-btn" data-sheet-close>ยกเลิก</button>
          <button class="btn primary feed-order-bulk-confirm-btn" type="submit" data-confirm-bulk-payment disabled>ยืนยันหักยอด</button>
        </div>
      </form>`, async (sheet) => {
          ...
          await withButton(previewBtn, 'กำลังคำนวณ...', async () => {
            ...
            if (resultBox) {
              ...
              requestAnimationFrame(() => {
                resultBox.scrollIntoView({ behavior: 'smooth', block: 'start', inline: 'nearest' });
              });
            }
